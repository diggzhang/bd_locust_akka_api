...
class WebsiteUser(HttpLocust):
    ...
    The `task_set` attribute should point to a TaskSet class
    which defines the behaviour of the user and is described
    in more detail below.

    In addition to the task_set attribute, one usually wants to declare
    the `min_wait` and `max_wait` attributes.
    These are the minimum and maximum time respectively,
    in milliseconds, that a simulated user will wait between executing
    each task. min_wait and max_wait default to 1000, and
    therefore a locust will always wait 1 second between each
    task if min_wait and max_wait are not declared.
